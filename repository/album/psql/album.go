...
func (repo *albumConnection) Create(ctx context.Context, album *entity.Album) (int64, error) {
	...
	err := repo.db.QueryRowContext(ctx, query, album.Title, album.ArtistID, album.Price).Scan(&album.ID)
	if err != nil {
		return 0, err
	}
...
func (repo *albumConnection) GetAllAlbum(ctx context.Context) ([]entity.Album, error) {
	// The query select
	query := `
		SELECT id, artist_id, title, price
		FROM album`
	// query := `
	// 	SELECT * FROM album INNER JOIN artists as a ON album.artist_id = artists.id`
...
func (repo *albumConnection) BatchCreate(ctx context.Context, albums []entity.Album) ([]int64, error) {
	...
	// The query insert
	query := `INSERT INTO album (title, artist_id, price) VALUES ($1, $2, $3) RETURNING id`
	...
	for _, album := range albums {
		...
		err := tx.QueryRowContext(ctx, query, album.Title, album.ArtistID, album.Price).Scan(&id)
		if err != nil {
...
func (repo *albumConnection) Update(ctx context.Context, album entity.Album) error {
	...
	// Run the query
	result, err := repo.db.ExecContext(ctx, query, album.Title, album.ArtistID, album.Price, album.ID)
	if err != nil {
		return err
	}
